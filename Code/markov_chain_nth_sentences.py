import random
import re
from parse_sentence_structure import format_text
...
class Markov_Chain(Dictogram):
    ...
    def get_starting_tuple(self):
        ...
        starting_tuple = tuple(tuple_words)
        return starting_tuple

    def walk_markov_chain(self):
        
        sentence = []
        sentence_length = random.randint(10,10)
        starting_tuple = self.get_starting_tuple()

        for word in starting_tuple:
            sentence.append(word)

        current_tuple = tuple(starting_tuple)
        
        next_word = self[current_tuple].sample()
        if next_word in "END START":
            sentence.append('')
        else:
        ...
        while (min_length == False) or (next_word != "END"):
